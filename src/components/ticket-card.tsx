...
export function TicketCard({ id, code, status: initialStatus, windowNumber }: TicketCardProps) {
  const [status, setStatus] = useState<"waiting" | "attending" | "completed">(initialStatus)

  const speakWindow = (wn: string) => {
    if (typeof globalThis !== "undefined" && "speechSynthesis" in globalThis) {
      const utterance = new SpeechSynthesisUtterance(
        `El cliente ${code} debe dirigirse a la ventanilla ${wn}`
      )
  ...
  const getStatusLabel = () => {
    switch (status) {
      case "waiting":
        return "Esperando"
      case "attending":
        return `Ventanilla ${windowNumber}`
      case "completed":
        return "Completado"
      default:
        return "Desconocido"
  ...
  return (
    <Card className={`p-4 border-2 flex flex-col items-center justify-center min-h-40 rounded-lg transition-all ${getStatusColor()}`}>
