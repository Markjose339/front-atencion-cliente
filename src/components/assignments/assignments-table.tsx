...
import { assignmentColumns } from "@/components/assignments/assignment-columns";
import { AlertCircle } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useRouter, useSearchParams } from "next/navigation";
import { useCallback } from "react";
import { useAssignmentsQuery } from "@/hooks/use-assignments";
...
export function AssignmentsTable() {
  ...
  const { findAllAssignments } = useAssignmentsQuery({ page, limit, search });
  const { data: response, isLoading, error, refetch } = findAllAssignments;
    ...
    return (
      <div className="flex flex-col items-center justify-center py-10 text-center text-red-500 space-y-4">
        ...
        <p className="text-sm font-medium">
          OcurriÃ³ un error: {(error as { message: string })?.message ?? "Error desconocido"}
        </p>
